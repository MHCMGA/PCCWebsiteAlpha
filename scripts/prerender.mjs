...
async function resolveChrome() {
  ...
  for (const p of candidates) {
    try {
      await stat(p);
      return { executablePath: p, args: [], headless: true };
    } catch {}
  }
...
async function main() {
  ...
  try {
    for (const route of ROUTES) {
      ...
      const cleanedHtml = html.replace(/<script[^>]*src="\/_vercel\/[^"]*"[^>]*><\/script>/g, "");
      ...
      console.log(
        `[prerender]   saved ${outPath}  (${bytes} bytes, title="${titleMatch?.[1] ?? ""}")`,
      );
      await page.close();
    }
  } finally {
